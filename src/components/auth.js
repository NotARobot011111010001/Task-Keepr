...
const Auth = () => {
    ...
    const handleLogin = async (type) => {
        const email = emailRef.current.value; // user email from useRef();
        const password = passwordRef.current.value; // user passoword from useRef();

        const { user, error } = 
        // waits for the async to receive a 'promise'
            type == "LOGIN"
    ...
    const forgotPassword = async (e) => {
        ...
        if (email === null || email === "") {
            setHelperText({ error: true, text: "No email entered. Please enter your email" });
        } else {
